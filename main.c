...
// Mobility Weight: How much a legal move is worth compared to board position
...
// IMPROVEMENT 1: STRATEGIC WEIGHTS
// Corners = +100 (Stable)
// X-Squares (b2, g2, b7, g7) = -50 (Very Dangerous)
// C-Squares (a2, b1, etc) = -20 (Risky)
...
// IMPROVEMENT 2: HYBRID EVALUATION (Static Weight + Mobility)
int Compute_Grades(int flag)
{
    int i, j;
    int B = 0, W = 0;
    ...
    // 1. Basic Counting and Static Weighted Score
    for (i = 0; i < Board_Size; i++)
        for (j = 0; j < Board_Size; j++)
        {
            ...
            {
                B++;
                Score_Black += board_weight[i][j];
            }
            else if (Now_Board[i][j] == 2) // White
            {
                W++;
                Score_White += board_weight[i][j];
            }
        }

    // 2. Display Mode (Flag == TRUE)
    // Just print the coin count as per rules
    if (flag)
    {
        Black_Count = B;
        White_Count = W;
    ...
    // 3. AI Thinking Mode (Flag == FALSE)
    // Here we include Mobility in the score.

    // We must count legal moves available for Black and White on the *current* board state.
    // Note: This is computationally expensive but necessary for good AI.
    // We cannot use the global Legal_Moves array as it belongs to the parent search node.
    // We use Find_Legal_Moves logic locally.
    ...
    for (i = 0; i < Board_Size; i++)
        for (j = 0; j < Board_Size; j++)
            if (Now_Board[i][j] == 0)
            {
                Now_Board[i][j] = 1; // Try Black
                if (Check_Cross(i, j, FALSE))
                    b_moves++;
                Now_Board[i][j] = 0; // Restore
            }
    ...
    // Positional Score + (Mobility Difference * Weight)
    int final_score = (Score_Black - Score_White) + ((b_moves - w_moves) * W_MOBILITY);
...
int Search(int myturn, int mylevel)
{
    ...
    // REMOVED RANDOM MOVE LOGIC
    // The new evaluation function is robust enough to play correctly from move 1.
    // Playing randomly at the start often gives away the game immediately.
